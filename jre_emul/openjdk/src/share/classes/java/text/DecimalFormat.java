...
public class DecimalFormat extends NumberFormat {
        ...
        for (int j = 1; j >= 0 && start < pattern.length(); --j) {
            ...
            for (int pos = start; pos < pattern.length(); ++pos) {
                ...
                case 1:
                    ...
                    if (ch == digit) {
                        ...
                        if (groupingCount > 0) {
                            secondaryGroupingCount = groupingCount;
                        }
                        groupingCount = 0;
            ...
            if ((decimalPos < 0 && digitRightCount > 0) ||
                (decimalPos >= 0 && (decimalPos < digitLeftCount ||
                 ...
                 groupingCount == 0 || secondaryGroupingCount == 0 || inQuote) {
            ...
            if (j == 1) {
                ...
                this.secondaryGroupingSize =
                    (secondaryGroupingCount > 0 && secondaryGroupingCount != groupingCount)
                    ? secondaryGroupingCount : 0;
    ...
    public void setCurrency(Currency currency) {
        if (currency != symbols.getCurrency()
            // J2ObjC: Added to be consistent with Android behavior.
    ...
    // J2ObjC: Adds support for Indian numbers that have different group sizes to match Android
    // support. (eg. 12,34,56,789)
    private byte secondaryGroupingSize = 0;
